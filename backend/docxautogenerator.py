...
def generate_fully_centered_patent_doc(
    ...
    logger.debug(f"📸 有效图片数量：{len(valid_img_paths)}")  # 新增：打印有效图片数，方便排查
    ...
    if fill_empty_space and table_h < half_page and valid_img_paths:
        ...
        if len(final_imgs) < needed_imgs_min:
            logger.debug(f"🔍 原始图片{len(final_imgs)}张不足，需补充到{needed_imgs_min}张")
            # 补充图片（重复原始图片）
            ...
            logger.debug(f"✅ 原始图片{len(final_imgs)}张足够，无需补充/截断")
    logger.debug(f"📸 最终插入图片数量：{len(final_imgs)}")  # 新增：打印最终插入数
    ...
    if final_imgs:
        ...
        logger.debug(f"📸 实际插入图片数量：{inserted_count}")  # 新增：验证插入数

        # 图片表格禁止跨页
        set_table_no_page_break(img_table)

    doc.save(save_path)
    logger.debug(f"✅ 文档生成成功：{os.path.abspath(save_path)}")
    return save_path
...
def generate_car_info_doc(car_data, save_path='公司车辆信息.docx', table_title='公司车辆信息'):
    ...
    # 保存文档
    doc.save(save_path)
    logger.debug(f"✅ 文档生成成功！路径：{os.path.abspath(save_path)}")
    return save_path
