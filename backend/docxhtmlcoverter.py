...
class DocxHtmlConverter:
    ...
    def _fix_html_img_sizes_for_import(self, html_text: str,
        ...
        def _process_img(tag: str, max_w_px: float) -> str:
            ...
            # ── max-width 处理 ───────────────────────────────────────────
            # style="max-width:100%; height:auto" 是"填满容器，保持比例"的语义。
            # CSS 里 max-width 不等于 width，style_dict.get('width') 取到的是空。
            # 修复：当 raw_w 仍为 None 且 style 里有 max-width 时，
            # 将 raw_w 设为上下文宽度（max_w_px），确保图片撑满容器而非用物理像素。
            if raw_w is None:
                max_w_str = style_dict.get('max-width', '').strip().lower()
                if max_w_str == '100%' or max_w_str.startswith('100'):
                    raw_w = max_w_px   # 已经是 px 单位的上下文宽度
                elif max_w_str:
                    mw = _css_val_to_px(max_w_str)
                    if mw:
                        raw_w = min(mw, max_w_px)
    ...
    def _extract_base64_images(self, html_text: str, base_dir: str):
        ...
        for m in matches:
            ...
            # 检查是否是 max-width:100% 的"填满容器"语义
            # 这类图片没有明确的目标宽度，需要由 _fix_html_img_sizes_for_import
            # 根据上下文容器宽度（<p> 用版心宽、<td> 用单元格宽）来决定。
            # 此处只做 src 替换和 MIME 校正，不写 width/height，交给后续处理。
            has_max_width_100 = False
            if style_m:
                style_val_check = re.sub(r'[\r\n]+\s*', ' ', style_m.group(1))
                mw_m = re.search(r'\bmax-width\s*:\s*100', style_val_check, re.IGNORECASE)
                has_max_width_100 = bool(mw_m) and not style_w_px and not attr_w_px

            # 确定最终 w/h（px）
            # style > HTML 属性 > 物理像素
            # max-width:100% 的图片不在此处锁定尺寸
            if has_max_width_100:
                w_px = h_px = None   # 交由 _fix_html_img_sizes_for_import 处理
            else:
